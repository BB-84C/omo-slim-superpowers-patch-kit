...
The installer registers the plugin in OpenCode's `opencode.json`. For manual
setups, add `oh-my-opencode-slim` to the `plugin` array in `opencode.json`.
This patched slim package does not ship a separate TUI companion plugin, so no
`tui.json` entry is required.
...
```jsonc
{
  ...
  "presets": {
    "openai": {
      ...
      "fixer": { "model": "openai/gpt-5.4-mini", "variant": "low", "skills": ["simplify"], "mcps": [] }
    },
    "opencode-go": {
      ...
      "fixer": { "model": "opencode-go/deepseek-v4-flash", "variant": "high", "skills": ["simplify"], "mcps": [] }
    }
  }
}
```

### Superpowers Lite Skills

To enable the Superpowers Lite bridge overlay, configure its active skills
...
Omitting `superpowersSkillsDir` disables the overlay. If the configured
directory is unavailable or unreadable, OMO warns and disables the overlay.
OMO does not probe `~/.config/opencode/superpowers/skills` and does not restore
a stock fallback roster.
