...
describe('isRateLimitError', () => {
  ...
  test('returns true for Insufficient balance message', () => {
    expect(isRateLimitError({ message: 'Insufficient balance.' })).toBe(true);
  });

  test('returns true for Insufficient balance response body', () => {
    expect(
      isRateLimitError({ data: { responseBody: 'Insufficient balance.' } }),
    ).toBe(true);
  });
